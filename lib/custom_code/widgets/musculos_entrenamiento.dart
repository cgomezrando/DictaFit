...
class MusculosEntrenamiento extends StatefulWidget {
  ...
  /// Documento de users/{uid}/entrenos. Si es null, muestra el último entreno.
  final DocumentReference? entrenoRef;

  @override
  State<MusculosEntrenamiento> createState() => _MusculosEntrenamientoState();
}

class _MusculosEntrenamientoState extends State<MusculosEntrenamiento> {
  ...
  static const Map<String, dynamic> _entrenoEjemplo = {
    'ejercicios': [
      {
        'nombre': 'Press de banca',
        'musculos': [
          {'musculo': 'pectoral_esternal', 'rol': 'principal', 'peso': 0.9},
          {'musculo': 'pectoral_clavicular', 'rol': 'principal', 'peso': 0.7},
          {'musculo': 'triceps', 'rol': 'secundario', 'peso': 0.5},
          {'musculo': 'deltoides_anterior', 'rol': 'secundario', 'peso': 0.4},
        ],
      },
      {
        'nombre': 'Dominadas',
        'musculos': [
          {'musculo': 'dorsal', 'rol': 'principal', 'peso': 0.9},
          {'musculo': 'biceps', 'rol': 'secundario', 'peso': 0.5},
          {'musculo': 'romboides', 'rol': 'secundario', 'peso': 0.4},
          {'musculo': 'deltoides_posterior', 'rol': 'secundario', 'peso': 0.3},
        ],
      },
    ],
  };

  Stream<Map<String, dynamic>?>? _entrenoStream;
  String _vista = 'frontal';
  bool _verEjemplo = false;
  ...
  @override
  void didUpdateWidget(covariant MusculosEntrenamiento oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.entrenoRef?.path != widget.entrenoRef?.path) {
      _crearStream();
    }
  }

  void _crearStream() {
    final ref = widget.entrenoRef;
    if (ref != null) {
      _entrenoStream =
          ref.snapshots().map((s) => s.data() as Map<String, dynamic>?);
      return;
    }
    ...
    _entrenoStream = FirebaseFirestore.instance
        .collection('users')
        .doc(uid)
        .collection('entrenos')
        ...
        .limit(1)
        .snapshots()
        .map((q) => q.docs.isEmpty ? null : q.docs.first.data());
  }
  ...
  Map<String, int> _nivelesMusculos(Map<String, dynamic> entreno) {
    ...
    for (final ejercicio in ejercicios) {
      ...
      for (final m in musculos) {
        ...
        final nivel = rol == 'principal' ? _nivelPrincipal : _nivelSecundario;
        if ((niveles[id] ?? 0) < nivel) niveles[id] = nivel;
      }
    }
    return niveles;
  }
  ...
  Widget _selectorVista() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          opcion('frontal', 'Frente'),
          opcion('dorsal', 'Espalda'),
        ],
      ),
    );
  }
  ...
  Widget _grupoMusculos(String titulo, List<String> ids, Color color) {
    final tema = FlutterFlowTheme.of(context);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
          ...
          children: ids.map((id) {
            return Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(999),
                border: Border.all(color: _tinte(color, 0.45)),
              ),
              child: Text(
                _nombres[id] ?? id,
                style: tema.bodySmall.copyWith(
  ...
  Widget _contenido(Map<String, dynamic> entreno) {
    ...
    final principales = niveles.entries
        ...
        .map((e) => e.key)
        .toList();
    final secundarios = niveles.entries
        .where((e) => e.value == _nivelSecundario)
        .map((e) => e.key)
        .toList();
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (niveles.isEmpty)
            Text(
              'Este entreno no tiene músculos registrados.',
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (principales.isNotEmpty) ...[
                    const SizedBox(height: 14),
                    _grupoMusculos('Principal', principales, _colorPrincipal),
                  ],
                  if (secundarios.isNotEmpty) ...[
                    const SizedBox(height: 14),
                    _grupoMusculos('Secundario', secundarios, _colorSecundario),
                  ],
                ],
              ),
            ),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.fitness_center_rounded, size: 48, color: tema.primary),
            ...
            const SizedBox(height: 6),
            Text(
              'Cuando dictes tu primer entreno verás aquí los músculos trabajados.',
              textAlign: TextAlign.center,
              style: tema.bodyMedium.copyWith(color: tema.secondaryText),
            ),
            const SizedBox(height: 20),
            OutlinedButton(
              onPressed: () => setState(() => _verEjemplo = true),
              style: OutlinedButton.styleFrom(
                foregroundColor: tema.primary,
                side: BorderSide(color: _tinte(tema.primary, 0.6)),
                shape: const StadiumBorder(),
                padding:
    ...
    return Container(
      ...
      child: SafeArea(
        child: StreamBuilder<Map<String, dynamic>?>(
          stream: _entrenoStream,
          builder: (context, snap) {
            final cargando = snap.connectionState == ConnectionState.waiting &&
                !snap.hasData;
            Map<String, dynamic>? entreno = snap.data;
            final esEjemplo = entreno == null && _verEjemplo;
            if (esEjemplo) entreno = _entrenoEjemplo;

            final subtitulo = entreno == null
                ? ''
                : esEjemplo
                    ? 'Entreno de ejemplo'
            ...
            if (cargando) {
              cuerpo = Center(
                child: CircularProgressIndicator(color: tema.primary),
              );
            } else if (entreno == null) {
              cuerpo = _estadoVacio();
            } else {
              cuerpo = _contenido(entreno);
            }
  ...
  // ---------- Geometría ----------
  // Vectorizada a partir de las siluetas de referencia (frente y espalda).
  // '_base' agrupa las zonas no entrenables (cabeza, manos, pies, tendones...).
